...
def entropy(dataset):
    ...
    #first, we have to count how many labels are there
    lst1 = [x[0] for x in dataset]
    label_list = list(set(lst1))
    n = dict()
    l = len(label_list)
    for i in range(l):
        n[label_list[i]] = i
    p = [0 for x_ in range(l)]
...
def gini_index(dataset):
    ...
    # sum ((pi)*(pj)) where i != j is equal to 1 - sum (pi^2)
    return 1 - s
...
def split_recurs(node, data, indices,  gain_function, max_depth):
    ...
    for attr_index in range(1,len(data[0])): # for each attribute
        # find all possible values in the feature (attribute)
        lst = [row[attr_index] for row in data]
        val_list = list(set(lst)) # unique possible values
        for val in val_list:
            g = calc_info_gain(data,attr_index,val,gain_function)
            lst_.append([g,attr_index,val])
    lst_.sort(reverse=True)
    #print(lst_)
    split_index = lst_[0][1]
    split_value = lst_[0][2]
